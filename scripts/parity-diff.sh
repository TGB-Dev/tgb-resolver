#!/bin/sh
# Live parity check: imports apps/server/testdata/parity.xml on both the
...
DATA_DIR="$(mktemp -d)"
PORT=5002 DATA_DIR="$DATA_DIR" go run ./apps/server >"$DATA_DIR/go.log" 2>&1 &
GO_PID=$!
trap 'kill $GO_PID' EXIT INT TERM
sleep 5

BODY="$(python3 -c "import json; print(json.dumps({'xml': open('$ROOT/apps/server/testdata/parity.xml').read()}))")"
