...
func NewHub(clock *Clock, verify func(token string) (string, error), logger *zerolog.Logger) *Hub {
	if verify == nil {
	...
	return &Hub{conns: map[*websocket.Conn]string{}, clock: clock, verify: verify, log: logger}
}
...
func (h *Hub) ServeHTTP(w http.ResponseWriter, r *http.Request) {
	...
	conns := len(h.conns)
	h.mu.Unlock()
	h.log.Info().Str("remote", r.RemoteAddr).Int("conns", conns).Msg("Hub client connected")
	defer func() {
		h.mu.Lock()
		delete(h.conns, c)
		remaining := len(h.conns)
		h.mu.Unlock()
		h.log.Info().Str("remote", r.RemoteAddr).Int("conns", remaining).Msg("Hub client disconnected")
	}()

	for {
		_, data, err := c.Read(r.Context())
		if err != nil {
			h.log.Debug().Err(err).Str("remote", r.RemoteAddr).Msg("Hub read closed")
			return
		}
		var req showv1.ClockSyncRequest
		if err := proto.Unmarshal(data, &req); err != nil {
			h.log.Warn().Err(err).Msg("Hub unmarshal ClockSyncRequest failed")
			continue
		}
		resp := h.HandleSyncClock(req.ClientTimeUnixMs)
		out, err := proto.Marshal(resp)
		if err != nil {
			h.log.Error().Err(err).Msg("Hub marshal ClockSyncResponse failed")
			continue
		}
		if err := c.Write(r.Context(), websocket.MessageBinary, out); err != nil {
			h.log.Debug().Err(err).Msg("Hub write ClockSyncResponse failed")
			return
		}
	}
}
...
func (h *Hub) Broadcast(env *showv1.Envelope) {
	h.log.Debug().Str("type", env.Type).Msg("Hub Broadcast start")
