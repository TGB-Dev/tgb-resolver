...
public sealed class AppJsonSerializerTests
{
    ...
    public async Task Serialize_UsesGeneratedContextForKnownTypes()
    {
        ...
        var payload = new ShowStateSnapshot(
            ...
            new PlaybackStateSnapshot(PlaybackStatus.Running, 1, 1, null, 120_000),
            new AssetCollectionSnapshot([], []),
            [new TimelineEventSnapshot(1, TimelineEventType.Res, 0, false, "Intro", new ResolveEventPayloadSnapshot("Alice", "alice", "A", 100, 1), null, null)]);

        var json = serializer.Serialize(payload);

        using var document = JsonDocument.Parse(json);
        await Assert.That(document.RootElement.GetProperty("showVersion").GetInt32()).IsEqualTo(3);
        await Assert.That(document.RootElement.GetProperty("meta").GetProperty("title").GetString()).IsEqualTo("Demo Show");
        await Assert.That(document.RootElement.GetProperty("playback").GetProperty("status").GetString()).IsEqualTo("Running");
    }
