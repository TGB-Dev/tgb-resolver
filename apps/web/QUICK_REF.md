...
`apps/web` is the canonical (and only) Vue frontend. It uses Vue 3.5 SFCs, Pinia setup stores, TanStack Vue
...
- **Prefer direct type annotations over `satisfies`** for `const` declarations that need a
  specific type: `const foo: readonly T[] = [...]`, not `const foo = [...] satisfies T`. A direct
  annotation gives the variable that exact type (so downstream `Record<K, V>` index access sees the
  intended shape); `satisfies` only checks and leaves a wider inferred type. Reserve `satisfies`
  for when you must preserve a narrower inferred type while still type-checking against a target.
- HTTP and realtime contracts come from `@tgb-resolver/contracts` and `@tgb-resolver/realtime`.
- **Keyboard-shortcut command system** (`src/features/shortcuts/`): command `scope` and binding
  `kind` MUST use the `CommandScope` and `CommandBindingKind` string-valued enums from `types.ts`
  (never raw string literals). New scopes/kinds require a new enum member plus updating the
  `satisfies` checks and discriminated unions.
