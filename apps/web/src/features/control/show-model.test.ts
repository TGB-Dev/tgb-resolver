...
test("hydrate with identical data keeps the store references stable", () => {
  showModel.hydrateFromSnapshot(makeShow(1, [event(1, 1), event(2, 2), event(3, 3)]));
  const idsBefore = showModel.showOrderedIds.peek();
  const eventsBefore = showModel.showEvents.peek();

  showModel.hydrateFromSnapshot(makeShow(1, [event(1, 1), event(2, 2), event(3, 3)]));

  // A big refetch (version gap / reconnect) with unchanged data must not
  // rebuild the ids array or the events map.
  expect(showModel.showOrderedIds.peek()).toBe(idsBefore);
  expect(showModel.showEvents.peek()).toBe(eventsBefore);
  expect(showModel.dataVersion.value).toBe(1);
});

test("hydrate applies a changed order as a new ids array", () => {
  showModel.hydrateFromSnapshot(makeShow(1, [event(1, 1), event(2, 2), event(3, 3)]));
...
test("hydrate applies changed event content as a new events map", () => {
  ...
  expect(showModel.showEvents.peek()).not.toBe(eventsBefore);
  expect((showModel.showEvents.value[2] as CustomEvent).payload.extId).toBe("updated");
});

test("hydrate applies a removed event as a new events map", () => {
  showModel.hydrateFromSnapshot(makeShow(1, [event(1, 1), event(2, 2)]));

  showModel.hydrateFromSnapshot(makeShow(2, [event(1, 1)]));

  expect(showModel.showEvents.value).toEqual({ 1: event(1, 1) });
});
...
test("retains table items when a snapshot only reorders events", () => {
  ...
  expect(showModel.showOrderedIds.value).toEqual([3, 1, 2]);
  // Order is carried by the ids array; a reorder-only snapshot must not
  // rebuild the events map (position is order-derived, not event content).
  expect(showModel.showEvents.peek()).toBe(eventsBefore);
...
test("skips the order write when a reorder echo matches the current order", () => {
  ...
  const applied = showModel.tryApplyShowMessage({
    ...
    showVersion: 3,
    orderedEventIds: [1, 2, 3],
  });

  expect(applied).toBe(true);
  // The echo carries the same order the optimistic apply already committed:
  // keep the array identity so the reorder list does not re-render.
  expect(showModel.showOrderedIds.peek()).toBe(idsBefore);
  // Version handling still advances.
  expect(showModel.dataVersion.value).toBe(3);
});
